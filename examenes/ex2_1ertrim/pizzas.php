...
<body>
    ...
    <form action="" method="post">
        <?php
        foreach ($productos as $producto => $pizzas) {
            if ($producto == "pizzas") {
                foreach ($pizzas as $pizza => $value) {
                    foreach ($value as $key => $valor) {
                        ...
                        if ($key == "precio") {
                            foreach ($valor as $tamanio => $precio) {
                                echo "<span>$tamanio -> $precio €</span>
                                <input type='number' name='cantidad[$tamanio]' value='' min='' max='10'><br>";
                            }
                        }
                    }
                }
